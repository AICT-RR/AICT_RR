...
const PORT = 5000;
app.use(cors()); 
app.use('/uploads', express.static(path.join(__dirname, '..', 'moreclient', 'data', 'data.json')));

// 파일 저장을 위한 설정
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        cb(null,  '../moreclient/public/uploads/'); // 파일을 저장할 경로
    },
...
app.delete('/delete/:fileName', (req, res) => {
    ...
    const filePath = path.join(__dirname, '..', 'moreclient', 'uploads', fileName);
...
const dataFilePath = path.join(__dirname, '..', 'moreclient', 'datadata/data.json');
